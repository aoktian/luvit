...
function clearBuffer(stream, state)
    ...
    if stream._writev and count(state.buffer) > 1 then
        --[[
    // Fast case, write everything using _writev()
    --]]
        local cbs = {}
        for c = 1, count(state.buffer) do
...
function needFinish(stream, state)
    return state.ending and state.length == 0 and count(state.buffer) == 0 and
